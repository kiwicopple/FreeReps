...
type DraftProtocol = Omit<NutritionProtocol, "profile" | "targets"> & {
  profile: Omit<
    NutritionProtocol["profile"],
  ...
  targets: Record<string, DraftTarget>;
};
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const protocol: NutritionProtocol = {
        ...
        profile: {
          ...draft.profile,
          age: requiredNumber(draft.profile.age, "Age"),
          height_cm: requiredNumber(draft.profile.height_cm, "Height"),
          weight_kg: requiredNumber(draft.profile.weight_kg, "Weight"),
        },
        targets: Object.fromEntries(
          Object.entries(draft.targets).map(([key, target]) => {
            if (
              target.kind === "range" &&
              (target.low === undefined || target.high === undefined)
            )
              throw new Error(
                `${nutrientLabel(key)} range bounds are required.`,
              );
            return [
              key,
              {
                ...target,
                value: requiredNumber(target.value, nutrientLabel(key)),
              },
            ];
          }),
        ),
  ...
  return (
    <section className="nutrition-editor" aria-label="Edit nutrition targets">
      ...
      <Form onSubmit={submit}>
        <Fieldset disabled={busy} aria-label="Nutrition targets">
          ...
          <Disclosure trigger={<> Profile and preferences </>}>
            <div className="nutrition-form-grid">
              ...
              <Field>
                <FieldLabel>Reference sex</FieldLabel>
                <Choice
            ...
            <p className="nutrition-muted">
              Changing profile details does not automatically recalculate
              targets.
            </p>
          </Disclosure>
              ...
              <div className="nutrition-form-grid">
                <Field>
                  ...
                  <NumericField
                    ...
                    value={t.value}
                    onValueChange={(value) => update(key, { value: value })}
                  />
                </Field>
                <Field>
                ...
                {t.kind === "range" && (
                  <>
                    <Field>
                      <FieldLabel>Range low</FieldLabel>
                      <NumericField
                        required
                    ...
                    <Field>
                      <FieldLabel>Range high</FieldLabel>
                      <NumericField
                        required
                ...
                {[
                  ...
                  <Field>
                    <FieldLabel>Total-intake upper limit (optional)</FieldLabel>
                    <NumericField
                      min={t.value ?? 0}
                      value={t.upper ?? null}
                      onValueChange={(value) =>
                        update(key, {
                          upper: value === null ? undefined : value,
                        })
                      }
                    />
                  </Field>
          ...
          <Field>
            <FieldLabel>Protocol notes</FieldLabel>
            <Textarea
              value={draft.notes}
