...
export default function SleepHeartRate({
  stages,
  compact = false,
}: {
  stages: SleepStage[];
  ...
  const path = points
    .map(
      (p, i) =>
        `${i === 0 || p.time - points[i - 1].time > 300000 ? "M" : "L"}${x(p.time)},${y(p.value)}`,
    )
    .join(" ");
  const overlay =
    show && lowest ? (
      <svg
        ...
        role="img"
        aria-label={`Heart rate in five-minute averages. Lowest ${lowest.value.toFixed(0)} beats per minute around ${formatClock(new Date(lowest.time).toISOString())}.`}
        style={{
          position: "absolute",
          inset: 0,
          width: "100%",
      ...
      >
        ...
        />
        <path
          d={`M${x(lowest.time)},${y(lowest.value)}h0.01`}
  ...
  return (
    <>
      ...
      {compact && valid && (
        ...
        >
          <span>{formatClock(new Date(start).toISOString())}</span>
          <span>{formatClock(new Date(end).toISOString())}</span>
        </div>
      )}
      <div
        aria-live="polite"
      ...
      >
        {query.isLoading ? (
          "Loading overnight heart rate…"
        ) : query.isError ? (
          <>
            Heart rate could not load.{" "}
          ...
          <>
            <strong style={{ color: "var(--foreground)" }}>
              Lowest 5-minute average: {lowest.value.toFixed(0)} bpm around{" "}
              {formatClock(new Date(lowest.time).toISOString())}
            </strong>
            <br />
            {Math.floor((lowest.time - start) / 3600000)}h{" "}
            {Math.floor((lowest.time - start) / 60000) % 60}m into the recorded
            night · {points.length} recorded intervals. Gaps indicate missing
            readings.
          </>
        ) : (
          "No heart-rate readings for this night yet."
        )}
      </div>
    </>
  );
