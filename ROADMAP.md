...
| Status | Item | Where | Trigger | Notes |
|---|---|---|---|---|
| `[open]` | Reconnect Withings through the consent flow | Settings → Withings tab | After `:edge` carries the `flexString` fix | The stored refresh token is dead: Withings rotated it on each of the 2,197 refreshes whose response FreeReps discarded between 2026-08-05 and 2026-09-20 ([`INCIDENTS.md`](INCIDENTS.md), 2026-09-20). The decode fix alone therefore yields `invalid_grant` rather than data. Check `GET /api/v1/import-logs` after the first sync cycle, and re-authorize if the error persists. |
