...
| `[open]` | Sleep stages and HR zones colour themselves from the brand token | `server/web/src/utils/stageColors.ts` | | `STAGE_COLOR.Awake` and the top heart rate zone read `--color-accent`, so data semantics carry the brand colour: on the hypnogram an awakening is drawn in the same blue as the nav marker and the selected range button. The colour roles split into brand, positive and negative on 2026-09-20 ([`DECISIONS.md`](DECISIONS.md)); a `--color-data-1…5` set is the remaining piece. Affects Sleep (hypnogram, stage composition) and Workout Detail (zone bars). |
